...
def extract_date(raw_date: str) -> DataTuple | None:
...
def is_same_month(first_date: DataTuple, second_date: DataTuple) -> bool:
    return first_date[1:] == second_date[1:]


def is_not_later(
    reference_date: DataTuple,
    transaction_date: DataTuple,
) -> bool:
    return transaction_date[::-1] <= reference_date[::-1]


def count_capital(reference_date: DataTuple) -> float:
...
def count_monthly_income(reference_date: DataTuple) -> float:
    ...
    for transaction in financial_transactions_storage:
        ...
        if not is_same_month(reference_date, transaction_date):
            continue
...
def count_monthly_expense(reference_date: DataTuple) -> float:
...
def count_categories(reference_date: DataTuple) -> dict[str, float]:
    ...
    for transaction in financial_transactions_storage:
        ...
        categories[category_name] = (
            categories.get(category_name, 0.0) + amount
        )

    return dict(sorted(categories.items()))


def print_capital(reference_date: DataTuple) -> str:
    capital = count_capital(reference_date)

    return f"{capital:.2f}"


def print_month_income(reference_date: DataTuple) -> str:
    monthly_income = count_monthly_income(reference_date)

    return f"{monthly_income:.2f}"


def print_month_expense(reference_date: DataTuple) -> str:
    monthly_expense = count_monthly_expense(reference_date)

    return f"{monthly_expense:.2f}"


def print_categories(reference_date: DataTuple) -> list[str]:
...
def stats_handler(date: str) -> str:
    ...
    month_profit = (
        count_monthly_income(parsed_date)
        - count_monthly_expense(parsed_date)
    )
